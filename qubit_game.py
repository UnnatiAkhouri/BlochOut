...
def update_system(bloch_vector, state_vector, gate_history, circuit_color):
    bloch_vector[:] = state_to_bloch_vector(state_vector)
    draw_circuit(gate_history, circuit_color)

# Function to draw quantum circuit
def draw_circuit(gate_history, color):
    circuit_fig = go.Figure()
    circuit_fig.add_trace(go.Scatter(
        x=[0, len(gate_history) + 1],
        y=[0, 0],
        mode="lines",
...
with col1:
    ...
    if st.button('X Gate (Qubit 1)'):
        ...
        update_system(bloch_vector1, st.session_state.state_vector1, st.session_state.gate_history1, 'pink')
...
with col2:
    ...
    if st.button('X Gate (Qubit 2)'):
        ...
        update_system(bloch_vector2, st.session_state.state_vector2, st.session_state.gate_history2, 'lightblue')
